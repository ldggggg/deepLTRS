...
###############################################################################
# loading notes
dat = np.load("sim_data_notes_75_60.npy")
M,P = dat.shape

# loading labels
clr = np.load('clr_75_60.npy')
clc = np.load('clc_75_60.npy')
...
with open ('sim_data_docs_75_60', 'rb') as fp:
    docs = pickle.load(fp)
...
for row, col in random.sample(ix, int(round(.5*len(ix)))):
    if np.random.uniform() < 1/4:    # validation
        ...
        #cdtm = np.delete(cdtm, row*P+col, axis = 0)
...
# val_test = np.concatenate((val_pos[0], test_pos[0]))
# unique_val_test = np.unique(val_test)
...
#### to solve   out = self.D2(F.relu(self.D1(val))) # dec 
    ...
    def decode_text(self, zV, zW):
        ...
        return out_p
        #return torch.zeros(size = (s1*s2, V))
...
def lossf(targetN, targetT, outN, outT, mu, logv, log_vareps, in_row = True):
    ...
    #         Z[row] = 0
    #nozero = len(np.where(targetRowSum.numpy()!=0)[0]) 
...
def train(epoch, store_muV, store_logv_V, store_muW, store_logv_W):    
    ...
    for batch_idx, obs in enumerate(Rdload):
        ...
        #for idx in range(b):
            ...
            # coo = sparse.coo_matrix(cdtm[idx*P:(idx+1)*P, :])
        # cdtm_u = cdtm_user(batch_idx*b, (batch_idx+1)*b)  ### old version
        cdtm_u = getWb(I, Wu, cdtm, b, batch_idx)
        #print(obs_text1.shape, cdtm_u.shape)
        #print(np.where(cdtm_u!=0))
        ...
        #obs_text1[idx*P:(idx+1)*P, :] = torch.sparse.FloatTensor(i, v, torch.Size(shape)).to_dense()
    ...
    for batch_idx, obs in enumerate(Cdload):
        ...
        #for idx in range(b):
            ...
            # coo = sparse.coo_matrix(cdtm[[idx for idx in range(idx, M*P, P)], :])
        #cdtm_p = cdtm_product(batch_idx*b, (batch_idx+1)*b)
        cdtm_p = getWb_col(I_col, Wu_col, cdtm_col, b, batch_idx)
        #print(batch_idx, b, cdtm_p.shape)
        #print(np.where(cdtm_p!=0))
        ...
        #obs_text1[idx*M:(idx+1)*M, :] = torch.sparse.FloatTensor(i, v, torch.Size(shape)).to_dense()
        #obs_text1 = torch.sparse.FloatTensor(i, v, torch.Size(shape)).to_dense()  
...
###################################
muV1 = store_muV[16]
logv_V1 = store_logv_V[16] 
muV2 = store_muV[13] 
logv_V2 = store_logv_V[13] 
muW1 = store_muW[15] 
logv_W1 = store_logv_W[15]

R1 = torch.zeros(size=(1, int_dim))
R2 = torch.zeros(size=(1, int_dim))
C1 = torch.zeros(size=(1, int_dim))

R1[:,:] = torch.normal.multivariate(mean = muV1, std = logv_V1, size = int_dim)
...
proba = Mod.decode_specific(R1, C1)
list_proba = proba.cpu().data.numpy().tolist
proba_dict = {}
