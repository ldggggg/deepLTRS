...
## This version introduces mini-batches
...
# *** 
# custom function to compare two pairs (here, arrays with 2 entries)
def IsTheFirstBigger(a, b):
    if a[0] > b[0]:
        return True
    elif a[0] == b[0]:
        if a[1] > b[1]:
...
# ***
# To reorganise the universal df(s) in row-major ordering (ROM): 
def CustomInsertionSort(M):
    for idx in range(1,len(M)):
        #print(idx)
        key = M.iloc[idx,0:2].copy()             # c'est complètement délirant..
        op = M.iloc[idx,2:].copy()
        j = idx-1
        while j>=0 and not IsTheFirstBigger(key, M.iloc[j,0:2]):
            M.iloc[(j+1),:] = M.iloc[j,:].copy()
            j= j-1
        M.iloc[(j+1),0:2] = key
        M.iloc[(j+1), 2:] = op
###############################################################################
...
# CustomInsertionSort(df)

##################################################
## Step 1: train, validation and test data sets ##
##################################################
import random
random.seed(0)
np.random.seed(0)
#interaction_pairs = np.vstack((df['Individual'].values, df['Object'].values)).transpose()
...
## valid and test
#selected_entries = random.sample(ix, int(round(.2*len(ix))))
#selected_entries = np.sort(selected_entries)
## train
#not_selected_entries = []
#for idx in ix:
#    if idx not in selected_entries:
#        not_selected_entries.append(idx)        
##        
#for entry in selected_entries:
#    row, col = interaction_pairs[entry,:].tolist()
#    if np.random.uniform() > 0.5:    # validation
#        entry_v.append(entry)
#    else:                            # test
#        entry_t.append(entry)    
#
#train_df = df.iloc[not_selected_entries,:]
#valid_df = df.iloc[entry_v,:]
#test_df = df.iloc[entry_t,:]        
#        
...
Ndload = DataLoader(Ndata,
                    batch_size = row_bs, # Ndata.__len__(),
                    shuffle = False
                    )
...
Rdload = DataLoader(Rdata,
                    batch_size = row_bs, # Rdata.__len__(), 
                    shuffle = False
                    )  
...
def train(epoch):    
    Mod.train()
    optimizer.zero_grad()      
    for batch_idx, inA in enumerate(Rdload):
        nrows = inA.shape[0]
        involved_rows = np.arange(batch_idx*row_bs, batch_idx*row_bs + nrows)
        involved_rows = involved_rows.tolist()
        #print(involved_rows)
        inN = itN.next()
        ...
        to_keep_bool = [idx in involved_rows for idx in obs_in_dat[0]]
        rows_to_keep = obs_in_dat[0][to_keep_bool]
        cols_to_keep = obs_in_dat[1][to_keep_bool]
        loss = lossf(inN[rows_to_keep, cols_to_keep],
                     inT[obs_in_dtm,:],
                     out_notes[rows_to_keep, cols_to_keep],
                     out_text[obs_in_dtm,:],
