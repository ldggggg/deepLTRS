...
for row, col in random.sample(ix, int(round(.96*len(ix)))):
    if np.random.uniform() > 25/96:    # validation
...
class Decoder(nn.Module):
    ...
    def forward(self, x):
        ...
        # out = self.D2(F.relu(self.D1(val))) # with decoder
        out = val # without decoder
...
acc_miss = accuracy_score(emnout, minA)
acc_obs = accuracy_score(eonout, oinN)
