...
###############################################################################
# loading notes
dat = np.load("C:/Users/Dingge/Doctoral_projets/Pytorch/sim_data_notes_patio.npy")
M,P = dat.shape

# loading texts
cdtm = np.load('C:/Users/Dingge/Doctoral_projets/Pytorch/cdtm_patio.npy')
cdtm_col = np.load('C:/Users/Dingge/Doctoral_projets/Pytorch/cdtm_col_patio.npy')
cdtm_ = cdtm.copy()
    ...
    def forward(self, x, idx, in_row = True):
        if in_row == True:
            ...
            if (idx+1)*batch_size_R < M:
                b = batch_size_R 
                b_u1 = self.b_u[idx*b:(idx+1)*b,:]
                b_u_ = bu_i[idx*b:(idx+1)*b,:]
            else:    # si pas divisible
                b = M - idx*batch_size_R   # le reste
                b_u1 = self.b_u[idx*batch_size_R:M,:]
                b_u_ = bu_i[idx*batch_size_R:M,:]
            #print(idx)
            #print(bu.size())
            #b_p = store_b_p 
            out_notes = self.decode_notes(V, W, b_u1, bp_j, b_u_, self.b_p)
            ...
            if (idx+1)*batch_size_C < P:
                b = batch_size_C 
                b_p1 = self.b_p[idx*b:(idx+1)*b,:]
                b_p_ = bp_j[idx*b:(idx+1)*b,:]
            else:    # si pas divisible
                b = P - idx*batch_size_C   # le reste
                b_p1 = self.b_p[idx*batch_size_C:P,:]
                b_p_ = bp_j[idx*batch_size_C:P,:]
            #b_u = store_b_u 
            out_notes = self.decode_notes(W, V, b_p1, bu_i, b_p_, self.b_u)
...
store_b_u = torch.Tensor(mean_u)  # initialization
store_b_p = torch.Tensor(mean_p)
...
for epoch in range(epochs):
    ...
    # valid check #
    ###############
    out = torch.mm(store_muV, store_muW.transpose(0,1)) + torch.mm(store_b_u, bp_j.transpose(0,1)) + torch.mm(bu_i, store_b_p.transpose(0,1))
...
## test
out_f = torch.mm(best_muV, best_muW.transpose(0,1)) + torch.mm(store_b_u, bp_j.transpose(0,1)) + torch.mm(bu_i, store_b_p.transpose(0,1))
...
torch.save(Mod.state_dict(), 'musical.pt')
np.save('Opti_best_muV_musical', best_muV)
np.save('Opti_best_muW_musical', best_muW)
np.save('Opti_best_logv_V_musical', best_logv_V)
np.save('Opti_best_logv_W_musical', best_logv_W )
np.save('Opti_vloss', store_vloss)
np.save('Opti_Rloss', store_Rloss)
np.save('Opti_Closs', store_Closs)
