...
for p in range(P):
    ...
    mean_p[p,:] = dat.T[p,:].sum()/float(nz*3)
if device=="cuda":
...
class Decoder(nn.Module):
    ...
    def decode_notes(self, zV, zW, bu, bp, bu_, bp_, b_all):
        ...
        bias = torch.mm(bu, bp.transpose(0,1)) + torch.mm(bu_, bp_.transpose(0,1)) + b_all
    ...
    def forward(self, x, idx, in_row = True):
        if in_row == True:
            ...
            if (idx+1)*batch_size_R < M:
                ...
                b_all1 = b_all[idx*b:(idx+1)*b,:]
            else:    # si pas divisible
                b = M - idx*batch_size_R   # le reste
                b_u1 = b_u[idx*batch_size_R:M,:]
                b_u_ = store_b_u[idx*batch_size_R:M,:]
                b_all1 = b_all[idx*batch_size_R:M,:]
            #print(idx)
            #print(bu.size())
            #b_p = store_b_p 
            out_notes = self.decode_notes(V, W, b_u1, store_b_p, b_u_, b_p, b_all1)
            ...
            if (idx+1)*batch_size_C < P:
                ...
                b_all1 = b_all.T[idx*b:(idx+1)*b,:]
            else:    # si pas divisible
                b = P - idx*batch_size_C   # le reste
                b_p1 = b_p[idx*batch_size_C:P,:]
                b_p_ = store_b_p[idx*batch_size_C:P,:]
                b_all1 = b_all.T[idx*batch_size_C:P,:]
            #b_u = store_b_u 
            out_notes = self.decode_notes(W, V, b_p1, store_b_u, b_p_, b_u, b_all1)
...
for epoch in range(epochs):
    ...
    # valid check #
    ###############
    out = torch.mm(store_muV, store_muW.transpose(0,1)) + torch.mm(b_u, bp_j.transpose(0,1)) + torch.mm(bu_i, b_p.transpose(0,1)) + b_all
...
## test
out_f = torch.mm(best_muV, best_muW.transpose(0,1)) + torch.mm(b_u, bp_j.transpose(0,1)) + torch.mm(bu_i, b_p.transpose(0,1)) + b_all
...
torch.save(Mod.state_dict(), 'musical_fix3.pt')
np.save('Fix3_best_muV_musical', best_muV)
np.save('Fix3_best_muW_musical', best_muW)
np.save('Fix3_best_logv_V_musical', best_logv_V)
np.save('Fix3_best_logv_W_musical', best_logv_W )
np.save('Fix3_vloss_musical', store_vloss)
np.save('Fix3_Rloss_musical', store_Rloss)
np.save('Fix3_Closs_musical', store_Closs)
